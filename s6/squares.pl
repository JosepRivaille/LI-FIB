...
doExample(E) :-
    ejemplo(E, Big, Sides),
    nl, write('Fitting all squares of size '), write(Sides), write(' into big square of size '), write(Big), nl,
    length(Sides, N), 
...
insideBigSquare(_, _, [], []).
insideBigSquare(N, B, [S|Sides], [V|Values]) :-
    insideBigSquare(N, B, Sides, Values),
    I in 1..Big,
    I #= V + S
